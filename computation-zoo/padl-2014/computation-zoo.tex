...
in the computation block. The \ident{let!} keyword which represents (monadic) 
...
\ident{return} keyword is mapped to the \emph{Return} operation:
%
\begin{equation*}
\begin{array}{l}
...
and the required type of $\emph{Bind}$ is $A\alpha \rightarrow (\alpha \rightarrow A\beta) \rightarrow A\beta$ 
...
the unit value in the \ident{false} case). Both have equivalent in the computation expression syntax:
%
\begin{equation*}
\begin{array}{l}
...
has a type $A\,\ident{unit} \rightarrow A\alpha \rightarrow A\alpha$ and corresponds to
...
typing of \emph{Zero} and \emph{Combine}. 
...
that, we need to rewrite code using combinators and use \ident{mplus}.
...
$[\alpha] \rightarrow [\alpha] \rightarrow [\alpha]$. \emph{For} behaves as monadic 
...
are often close to built-in syntax. The author of a concrete computation (\ident{parse}, 
...
This is captured by monad transformers \cite{monad-transformers}. In F\#, they are not supported
directly, but they still provide a useful conceptual framework. 
...
follow the list transformer \cite{list-trans} and define the type as:
%
\begin{equation*}
\begin{array}{lcl}
...
\end{equation*}
%
The computations composed using parallel binding are combined using \emph{Merge}. This defines the 
structure used for HTML rendering. The rest of the computation is turned into a pure function passed to \emph{Map}.
Note that the translation allows uses beyond applicative functors. The \ident{let!}\ldots\ident{and}\ldots
syntax can be used with monads to write zip comprehensions \cite{monad-compre}.
...
In this paper, we relate computation expressions to standard abstract computation types. In 
\S\ref{sec:semantics-typing}, we present new typing rules that capture such common uses and would 
make the system more robust by supporting better error messages and disallowing non-standard uses.
